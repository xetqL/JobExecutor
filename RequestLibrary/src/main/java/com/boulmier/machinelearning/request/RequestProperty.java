...
public abstract class RequestProperty implements Serializable{
    ...
    public static final Property<String,String> 
            EXECUTABLE_NAME = new Property<>("executable",NULL),
    ...
    public static Property<String,String> fromString(String s){
        if(s.equals(EXECUTABLE_NAME.getA())) return EXECUTABLE_NAME;
        if(s.equals(JOB_NAME.getA())) return JOB_NAME;
        if(s.equals(JOB_IDENTIFIER.getA())) return JOB_IDENTIFIER;
        if(s.equals(CLIENT_EMAIL.getA())) return CLIENT_EMAIL;
        if(s.equals(NUMBER_OF_HIDDEN_NEURONS.getA())) return NUMBER_OF_HIDDEN_NEURONS;
        if(s.equals(NUMBER_OF_INPUT_NEURONS.getA())) return NUMBER_OF_INPUT_NEURONS;
        if(s.equals(TRAINING_SET_URI.getA())) return TRAINING_SET_URI;
        if(s.equals(TESTING_SET_URI.getA())) return TESTING_SET_URI;
        if(s.equals(NUMBER_OF_EPOCHS.getA())) return NUMBER_OF_EPOCHS;
        if(s.equals(ARGS.getA())) return ARGS;
        return null;
    }
